...
LOG_DATA_SOURCE_S3      = config.get('S3', 'LOG_DATA')
LOG_JSONPATH_SOURCE_S3  = config.get('S3', 'LOG_JSONPATH')
SONG_DATA_SOURCE_S·    = config.get('S3', 'SONG_DATA')
...
# Data copy from the S3 data bucket
# Please, note that we need to add the Redshift role
staging_events_copy = ("""
    COPY staging_events FROM {data_bucket}
    credentials 'aws_iam_role={role_arn}'
    region 'us-west-2' format as JSON {log_json_path}
    timeformat as 'epochmillisecs';
""").format(data_bucket=config['S3']['LOG_DATA'], role_arn=config['IAM_ROLE']['ARN'],region=config['CLUSTER']['REGION']
...
# FINAL TABLES
# Insert data from the stagging tables
...
# Please, note the extraction of the time components from the 
# start_time property 
time_table_insert = ("""
